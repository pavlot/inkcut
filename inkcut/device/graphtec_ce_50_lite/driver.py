...
class StepperMotor(Model):
    ...
    def step(self, steps):
        ...
        i = 0
        while i < n:
            dt = (self.clock_time + (self.delay / 1000000.0)) - time.time()
            # Do nothing # This is effectively time.sleep() however the sleep
            # time is adjusted based on how long it takes to execute other
            # sections of code
            time.sleep(dt)

            # Set previous clock time to current time
            ...
            # DRV8825 requires a minium high pulse of 2us making 3us for margin
            time.sleep(0.000003)
...
class PiDevice(Device):
    ...
    def init_motors(self, change):
        ...
        config = self.config
        # Init motors
        self.motor[0] = StepperMotor(
            driver_pins=config.motor_driver_pins[0],
            enable_pin=config.motor_enable_pins[0],
            delay=config.delay,
        )
        self.motor[1] = StepperMotor(
            driver_pins=config.motor_driver_pins[1],
    ...
    def init_boundary_sensors(self, pins=None):
        ...
        # Init boundary pins
        for i in range(2):
            GPIO.setup(self.boundary_gpio_pins[i][0], GPIO.IN, pull_up_down=GPIO.PUD_UP)
            GPIO.setup(self.boundary_gpio_pins[i][1], GPIO.IN, pull_up_down=GPIO.PUD_UP)

        # Init callbacks
        bbt = self.boundary_bounce_timeout
        GPIO.add_interrupt_callback(
            self.boundary_gpio_pins[0][0], callback=self.on_hit_bound_min_x, debounce_timeout_ms=bbt
        )
        GPIO.add_interrupt_callback(
            self.boundary_gpio_pins[0][1], callback=self.on_hit_bound_max_x, debounce_timeout_ms=bbt
        )
        GPIO.add_interrupt_callback(
            self.boundary_gpio_pins[1][0], callback=self.on_hit_bound_min_y, debounce_timeout_ms=bbt
        )
        GPIO.add_interrupt_callback(
            self.boundary_gpio_pins[1][1], callback=self.on_hit_bound_max_y, debounce_timeout_ms=bbt
        )
